# evaluator.py
"""
Evaluator for Kaggle Playground: Predict Calorie Expenditure using OpenEvolve.

Design:
- Every evaluation run samples a fresh random validation subset from train.csv.
- We score with RMSLE = RMSE on log1p(Calories) to match the competition metric.
...
def _rmsle(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """RMSLE = RMSE between log1p(true) and log1p(pred)."""
...
try:
    ...
    y_true_val = train_df.iloc[val_idx]["Calories"].values.astype(float)
...
def evaluate(program_path: str) -> dict:
    ...
    try:
        ...
        print(f"Evaluation: RMSLE={out['rmsle']:.6f} | score={out['combined_score']:.6f} | n_train={out['n_train']} n_val={out['n_val']} | time={out['eval_time']:.2f}s")
        return out
...
def evaluate_stage1(program_path: str) -> dict:
    ...
    sample_rows = 5000  # quick subset for speed
    ...
    try:
        ...
        print(f"[Stage1] RMSLE={out['rmsle']:.6f} | score={out['combined_score']:.6f} | n_train={out['n_train']} n_val={out['n_val']} | time={out['eval_time']:.2f}s")
        return out
